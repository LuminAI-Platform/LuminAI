 ...
 *
 * <p>This project's multi-tenancy is schema-based: {@link TenantContext} holds the current
 * tenant's schema id, and Hibernate routes every connection to that schema before any query in
 * this service runs (see {@link TenantContext}'s own Javadoc — it's read by {@code
 * TenantIdentifierResolver}). Because isolation is physical, repository calls here are plain
 * {@code findById}/{@code findAll} lookups with no manual tenant filter — there's nothing to
 * filter by that Hibernate hasn't already resolved for the connection. {@link TenantContext} is
 * only read here to stamp the tenant id onto outgoing Kafka events, since consumers on a shared
 * topic need to know which tenant an event belongs to.
 *
...
public class MergeReviewService {
    ...
    public Page<MergeReviewDto.CandidateResponse> listCandidates(CandidateStatus status, Pageable pageable) {
        Page<ErCandidate> page =
                status != null ? candidateRepository.findByStatus(status, pageable) : candidateRepository.findAll(pageable);
    ...
    /**
     * Accepts a candidate match: merges Record B's non-conflicting properties into Record A's
     * Golden Record, updates provenance, marks the candidate ACCEPTED, and publishes an
    ...
    /**
     * Rejects a candidate match. The candidate row is preserved (never deleted) so the ER engine
     * does not re-propose the same pair on a future pipeline run.
     */
    @Transactional
     ...
     */
    @Transactional
    public MergeReviewDto.SplitResponse splitGoldenRecord(UUID goldenRecordId, UUID sourceRecordIdToExtract) {
        ...
        if (!original.getSourceRecordIds().contains(sourceRecordIdToExtract)) {
            throw new ConflictException(
                    "Source record " + sourceRecordIdToExtract + " is not part of Golden Record " + goldenRecordId);
        }
        ...
        original
                .getProvenance()
                .add(new ProvenanceEntry(sourceRecordIdToExtract, null, null, Instant.now(), "SPLIT_REMOVE"));
        ...
        extracted
                .getProvenance()
                .add(new ProvenanceEntry(sourceRecordIdToExtract, null, null, Instant.now(), "SPLIT_CREATE"));
     ...
     * disagree are left untouched — resolving true conflicts is left to existing business rules
     * (e.g. a separate manual field-level override flow), consistent with the project's existing
     * merge semantics if one is already defined.
     */
        ...
        for (Map.Entry<String, Object> entry : incoming.entrySet()) {
            ...
            if (noConflict) {
                ...
                goldenRecord
                        ...
                        .add(
                                new ProvenanceEntry(
                                        candidate.getRecordBId(), field, candidate.getId(), Instant.now(), "FIELD_MERGE"));
            }
