...
/**
 * Local development implementation of a credentials vault.
 * Encrypts credentials using AES-256-GCM before storing them in memory.
 *
 * <p><strong>Design note:</strong> This class is intentionally interface-ready.
 * For production, replace this with an AWS Secrets Manager or HashiCorp Vault
 * implementation that implements the same public method signatures.
 *
 * <p>Credentials are keyed by {@code tenantId + connectorId} to ensure
...
public class CredentialsVaultService {
     ...
     * Key: "tenantId::connectorId", Value: Base64-encoded ciphertext (IV + encrypted data).
     */
    ...
    /**
     * Normalise the encryption key to exactly 32 bytes (AES-256).
