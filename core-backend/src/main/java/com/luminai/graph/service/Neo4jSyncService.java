 ...
 * producer emits, so it's derived from the event's free-form {@code data} map with a fallback
 * chain ({@code data.canonical_name} → {@code data.name} → {@code golden_id}).
 *
 ...
 * supported — each entry expected to carry {@code target_id}/{@code targetId}, {@code type}, and
 * an optional {@code direction} — and synced verbatim if present. If absent, which is the case for
...
public class Neo4jSyncService {
    ...
    public void sync(EntityResolvedEvent event) {
        ...
        repository.mergeEntityNode(event.tenantId(), event.goldenId(), canonicalName, event.entityType());
        log.info(
