...
from app.telemetry import record_pipeline_duration, trace_span
...
class DagsterTrigger:
    ...
    def trigger_cleaning_pipeline(
        ...
        try:
            with trace_span(
                "pipeline.orchestration.cleaning",
                attributes={
                    "tenant.id": str(tenant_id),
                    "source.id": str(source_id),
                    "run.id": str(active_run_id),
                    "trigger": "kafka_batch_complete",
                },
            ):
                ...
                if result.success:
                    ...
                    logger.info(
                        "✅ Cleaning pipeline completed — tenant=%s, source=%s, run_id=%s, dagster_run_id=%s (took %.2fs)",
                        tenant_id,
                        source_id,
                        active_run_id,
                        dagster_run_id,
                    ...
                    duration = time.perf_counter() - start_time
                    record_pipeline_duration("cleaning_pipeline", duration, status="failed")
                    tracker.fail_run(
                        active_run_id,
        ...
        try:
            with trace_span(
                ...
                if result.success:
                    ...
                    logger.info("✅ ER pipeline completed — tenant=%s, run_id=%s, dagster_run_id=%s (took %.2fs)", tenant_id, active_run_id, dagster_run_id, duration)
                    return dagster_run_id
                else:
                    duration = time.perf_counter() - start_time
                    record_pipeline_duration("er_pipeline", duration, status="failed")
                    tracker.fail_run(
                        active_run_id,
