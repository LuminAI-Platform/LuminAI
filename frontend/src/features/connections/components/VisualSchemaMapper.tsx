...
export const VisualSchemaMapper: React.FC<VisualSchemaMapperProps> = ({
  ...
  const [savedPayload, setSavedPayload] = useState<SchemaMappingPayload | null>(null);
  ...
  const onDropOnProp = (propId: string) => {
    ...
    setMappings((prev) => {
      ...
      const filtered = prev.filter(
        (m) => m.sourceColumnId !== draggingColId && m.ontologyPropertyId !== propId
      );
      return [...filtered, { sourceColumnId: draggingColId, ontologyPropertyId: propId, transform: "direct" }];
    });
    setDraggingColId(null);
    setHoverPropId(null);
  ...
  const removeMapping = (sourceColumnId: string) => {
    setMappings((prev) => prev.filter((m) => m.sourceColumnId !== sourceColumnId));
    setTimeout(recalcLines, 50);
  };
    ...
    <div className="flex flex-col h-full gap-0 select-none">
      ...
      <div className="flex items-center justify-between px-6 py-4 border-b border-zinc-800/80 bg-zinc-900/60 backdrop-blur flex-shrink-0">
        <div className="flex items-center gap-3">
          ...
          <span className="text-xs text-blue-400 font-mono">{targetEntity}</span>
        </div>
        <div className="flex items-center gap-3">
          {/* Stats pills */}
      ...
      {showPayload && savedPayload && (
        <div className="px-6 py-3 border-b border-zinc-800/80 bg-black/60 flex-shrink-0">
          <p className="text-[10px] text-zinc-500 mb-1.5 font-mono uppercase tracking-widest">API Payload Config</p>
      ...
      >
        ...
        <div className="w-1/2 flex flex-col border-r border-zinc-800/80 overflow-hidden">
          <div className="px-5 py-3 border-b border-zinc-800/60 flex items-center justify-between flex-shrink-0">
            <div>
              <p className="text-xs font-semibold text-zinc-200">Source Columns</p>
              <p className="text-[10px] text-zinc-500 mt-0.5">Drag a row to connect it to an ontology property</p>
            </div>
          ...
          <div className="overflow-y-auto flex-1 p-4 flex flex-col gap-2">
            {sourceColumns.map((col) => {
              ...
              const mappedProp = ontologyProperties.find((p) => p.id === mappedPropId);
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1">
                      <span className="text-xs font-semibold text-zinc-100 font-mono truncate">{col.name}</span>
                      {col.nullable && (
                    ...
                    {col.sample && (
                      <p className="text-[10px] text-zinc-500 font-mono truncate">e.g. {col.sample}</p>
                    )}
                  </div>

                  {/* Type badge */}
                  <span className={`text-[9px] font-semibold px-2 py-0.5 rounded border flex-shrink-0 ${DATA_TYPE_COLORS[col.dataType] ?? "text-zinc-400 bg-zinc-900 border-zinc-800"}`}>
                    {col.dataType}
                  </span>
                  ...
                  {isMapped && (
                    <div className="flex items-center gap-1.5 flex-shrink-0">
                      <span className="text-[9px] text-blue-400 font-mono truncate max-w-[80px]">{mappedProp?.name}</span>
                      <button
                        onClick={(e) => { e.stopPropagation(); removeMapping(col.id); }}
                  ...
                  <div className={`w-2.5 h-2.5 rounded-full border-2 flex-shrink-0 transition-all ${
                    isMapped ? "bg-blue-500 border-blue-400" : "bg-zinc-800 border-zinc-700 group-hover:border-blue-500/60"
                  }`} />
                </div>
              );
            })}
        ...
        <div className="w-1/2 flex flex-col overflow-hidden">
          <div className="px-5 py-3 border-b border-zinc-800/60 flex items-center justify-between flex-shrink-0">
            <div>
              <p className="text-xs font-semibold text-zinc-200">Ontology Properties</p>
              <p className="text-[10px] text-zinc-500 mt-0.5">Drop a source column onto a property to map it</p>
            </div>
            <span className="text-[10px] font-mono text-zinc-600 px-2 py-0.5 bg-zinc-900 border border-zinc-800 rounded">
              {targetEntity}
            </span>
            ...
            {ontologyProperties.map((prop) => {
              ...
              return (
                <div
                  ...
                  onDragOver={(e) => { e.preventDefault(); setHoverPropId(prop.id); }}
                ...
                >
                  {/* Left connector dot */}
                  <div className={`w-2.5 h-2.5 rounded-full border-2 flex-shrink-0 transition-all ${
                    isMapped ? "bg-blue-500 border-blue-400" : isHovered ? "bg-blue-500/50 border-blue-500" : "bg-zinc-800 border-zinc-700"
                  }`} />

                  {/* Prop info */}
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-0.5">
                      <span className="text-xs font-semibold text-zinc-100 font-mono truncate">{prop.name}</span>
                      {prop.required && (
                    ...
                    {prop.description && (
                      <p className="text-[10px] text-zinc-500 truncate">{prop.description}</p>
                    )}
                  </div>

                  {/* Expected type */}
                  <span className="text-[9px] text-zinc-500 font-mono flex-shrink-0">{prop.expectedType}</span>
                  ...
                  {isMapped ? (
                    ...
                    <span className="text-[9px] text-zinc-600 flex-shrink-0 animate-pulse">drop here</span>
                  ) : null}
                </div>
              );
            })}
      ...
      <div className="px-6 py-3 border-t border-zinc-800/80 bg-zinc-900/40 flex items-center justify-between flex-shrink-0">
        <div className="flex items-center gap-4 text-[10px] text-zinc-500">
          <span>{mappings.length} of {sourceColumns.length} source columns mapped</span>
          <span>·</span>
          <span>{ontologyProperties.filter((p) => p.required && !getMappedColId(p.id)).length} required properties unfilled</span>
        </div>
