...
function emptyProperty(): PropertySchema {
  return { id: generateId(), name: "", type: "STRING", required: false, defaultValue: "", description: "" };
}

export const PropertySchemaForm: React.FC<PropertySchemaFormProps> = ({ properties, onChange }) => {
  ...
  return (
    <div className="flex flex-col gap-2">
      <div className="flex items-center justify-between mb-1">
        <span className="text-xs font-semibold text-zinc-300 uppercase tracking-widest">Properties</span>
        <button
          type="button"
          id="add-property-btn"
          onClick={addProperty}
      ...
      {properties.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-8 border border-dashed border-zinc-800 rounded-xl text-center">
          <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" className="text-zinc-600 mb-2">
            <path d="M9 5H7a2 2 0 0 0-2 2v12a2 2 0 0 0 2 2h10a2 2 0 0 0 2-2V7a2 2 0 0 0-2-2h-2" />
            <rect x="9" y="3" width="6" height="4" rx="1" />
          </svg>
          <span className="text-xs text-zinc-500 font-medium">No properties defined</span>
          <span className="text-[10px] text-zinc-600 mt-0.5">Click "Add Property" to define schema fields</span>
        </div>
      ) : (
        <div className="flex flex-col gap-1.5">
          {properties.map((prop) => {
            ...
            return (
              <div key={prop.id} className="border border-zinc-800/80 rounded-xl overflow-hidden bg-zinc-950/50">
                <div
                ...
                >
                  ...
                  <span className="text-xs font-semibold text-zinc-200 flex-1 font-mono truncate">
                    {prop.name || <span className="text-zinc-600 font-normal italic">Unnamed property</span>}
                  </span>
                  {prop.required && (
                    <span className="text-[9px] font-bold text-red-400 bg-red-500/10 border border-red-500/20 px-1.5 py-0.5 rounded shrink-0">REQUIRED</span>
                  )}
                  <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"
                  ...
                  <div className="px-4 pb-4 pt-2 border-t border-zinc-800/60 flex flex-col gap-3">
                    <div className="grid grid-cols-2 gap-3">
                      <div className="flex flex-col gap-1">
                        <label className="text-[10px] font-semibold text-zinc-500 uppercase tracking-wider">Property Name *</label>
                        <input id={`prop-name-${prop.id}`} type="text" value={prop.name}
                      ...
                      <div className="flex flex-col gap-1">
                        <label className="text-[10px] font-semibold text-zinc-500 uppercase tracking-wider">Data Type *</label>
                        <select id={`prop-type-${prop.id}`} value={prop.type}
                          onChange={(e) => update(prop.id, { type: e.target.value as PropertyType })}
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div className="flex flex-col gap-1">
                        <label className="text-[10px] font-semibold text-zinc-500 uppercase tracking-wider">Default Value</label>
                        <input id={`prop-default-${prop.id}`} type="text" value={prop.defaultValue}
                          onChange={(e) => update(prop.id, { defaultValue: e.target.value })}
                          placeholder="Optional default..."
                          className="px-2.5 py-2 bg-zinc-900 border border-zinc-800 rounded-lg text-xs text-zinc-200 outline-none focus:border-blue-500/60 font-mono placeholder:text-zinc-600 transition-colors" />
                      </div>
                      <div className="flex flex-col gap-1">
                        <label className="text-[10px] font-semibold text-zinc-500 uppercase tracking-wider">Required</label>
                        <div className="flex items-center gap-3 h-[34px]">
                          <button type="button" id={`prop-required-${prop.id}`}
                            onClick={() => update(prop.id, { required: !prop.required })}
                            ...
                            <span className={`absolute top-0.5 w-4 h-4 rounded-full bg-white shadow transition-all duration-200 ${prop.required ? "left-5" : "left-0.5"}`} />
                          </button>
                          <span className={`text-xs font-medium ${prop.required ? "text-blue-400" : "text-zinc-500"}`}>
                    ...
                    <div className="flex flex-col gap-1">
                      ...
                      <input id={`prop-desc-${prop.id}`} type="text" value={prop.description}
                    ...
                    <div className="flex justify-end pt-1">
                      <button type="button" id={`prop-delete-${prop.id}`} onClick={() => remove(prop.id)}
