...
export const OntologyPage: React.FC = () => {
  ...
  const deleteEntity = async (id: string) => {
    ...
    try {
      await apiFetch(`/api/v1/ontology/entity-types/${id}`, { method: "DELETE" });
      setEntities((prev) => prev.filter((e) => e.id !== id));
    } catch { setError("Failed to delete entity type."); }
  };

  const deleteRelationship = async (id: string) => {
    if (!confirm("Delete this relationship type?")) return;
    try {
      await apiFetch(`/api/v1/ontology/relationship-types/${id}`, { method: "DELETE" });
      setRelationships((prev) => prev.filter((r) => r.id !== id));
    } catch { setError("Failed to delete relationship type."); }
  };

  const publishVersion = async () => {
    setPublishing(true);
    try {
      const res = await apiFetch("/api/v1/ontology/versions", { method: "POST" });
      const v = await res.json() as OntologyVersion;
      setVersions((prev) => [v, ...prev]);
      setTab("versions");
    } catch { setError("Failed to publish ontology version."); }
  ...
  return (
    <div className="flex flex-col gap-0 h-full overflow-hidden">
      ...
      <div className="flex items-start justify-between pb-5 shrink-0">
        <div>
          <h1 className="text-xl font-semibold text-zinc-100">Ontology Schema Editor</h1>
          <p className="text-xs text-zinc-500 mt-1">Define entity types, properties, and relationship topology for the knowledge graph</p>
        </div>
        <button
      ...
      <div className="grid grid-cols-3 gap-3 mb-5 shrink-0">
        {[
          { label: "Entity Types", value: entities.length, color: "#3b82f6", icon: <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="12" cy="8" r="4"/><path d="M6 20v-2a6 6 0 0 1 12 0v2"/></svg> },
          { label: "Relationship Types", value: relationships.length, color: "#a855f7", icon: <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6"/><polyline points="15 3 21 3 21 9"/><line x1="10" y1="14" x2="21" y2="3"/></svg> },
          { label: "Total Properties", value: entities.reduce((s, e) => s + (e.properties?.length ?? 0), 0), color: "#10b981", icon: <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M9 5H7a2 2 0 0 0-2 2v12a2 2 0 0 0 2 2h10a2 2 0 0 0 2-2V7a2 2 0 0 0-2-2h-2"/><rect x="9" y="3" width="6" height="4" rx="1"/></svg> },
        ].map((s) => (
          <div key={s.label} className="flex items-center gap-3 px-4 py-3 bg-zinc-900/60 border border-zinc-800/80 rounded-xl">
            <div className="w-8 h-8 rounded-lg flex items-center justify-center shrink-0"
              style={{ backgroundColor: `${s.color}18`, color: s.color }}>
              {s.icon}
            </div>
            <div>
              <div className="text-lg font-bold text-zinc-100 leading-none">{s.value}</div>
      ...
      <div className="flex border-b border-zinc-800/80 shrink-0 mb-5">
        {(["entities", "relationships", "versions"] as Tab[]).map((t) => (
          <button key={t} id={`ontology-tab-${t}`} type="button" onClick={() => setTab(t)}
            ...
            {t === "entities" ? `Entity Types` : t === "relationships" ? "Relationship Types" : "Published Versions"}
          </button>
        ))}
      </div>
      ...
      {error && (
        <div className="mb-4 px-4 py-3 bg-red-950/30 border border-red-500/20 text-red-400 rounded-xl text-xs flex items-center justify-between shrink-0">
          <span>{error}</span>
          <button onClick={() => setError(null)} className="underline hover:text-red-300 cursor-pointer">Dismiss</button>
        </div>
      )}
      ...
      <div className="flex-1 overflow-y-auto pr-1 pb-6">
        ...
        {tab === "entities" && (
          <div className="flex flex-col gap-4">
            <div className="flex justify-end">
              <button id="new-entity-btn" type="button" onClick={() => { setEditingEntity(undefined); setEntityEditorOpen(true); }}
            ...
            {loading ? (
              <div className="flex items-center justify-center gap-2 py-12 text-zinc-500 text-xs">
                <Spinner /><span>Loading entity types…</span>
              </div>
            ) : entities.length === 0 ? (
              <EmptyState
                icon={<svg width="36" height="36" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5"><circle cx="12" cy="8" r="4"/><path d="M6 20v-2a6 6 0 0 1 12 0v2"/></svg>}
                title="No entity types defined yet"
                sub="Create your first entity type to start modelling your domain"
                action={
                  <button onClick={() => { setEditingEntity(undefined); setEntityEditorOpen(true); }}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                {entities.map((entity) => (
                  <div key={entity.id ?? entity.name}
                    ...
                    <div className="flex items-start justify-between">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <span className="text-[10px] font-mono text-zinc-500 block mt-0.5">{entity.name}</span>
                        </div>
                      </div>
                      {/* Actions */}
                      <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                        <button id={`edit-entity-${entity.id}`} type="button"
                          onClick={() => { setEditingEntity(entity); setEntityEditorOpen(true); }}
                          className="p-1.5 text-zinc-500 hover:text-blue-400 hover:bg-zinc-800 rounded-lg transition-all cursor-pointer" title="Edit">
                        ...
                        {entity.id && (
                          <button id={`delete-entity-${entity.id}`} type="button" onClick={() => deleteEntity(entity.id!)}
                            className="p-1.5 text-zinc-500 hover:text-red-400 hover:bg-zinc-800 rounded-lg transition-all cursor-pointer" title="Delete">
                            <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                    ...
                    {/* Description */}
                    {entity.description && (
                      <p className="text-[11px] text-zinc-500 leading-normal line-clamp-2">{entity.description}</p>
                    )}
                    ...
                    {entity.properties && entity.properties.length > 0 && (
                      <div className="flex flex-wrap gap-1.5">
                        {entity.properties.slice(0, 5).map((p) => (
                          <span key={p.id} className="text-[9px] font-mono font-semibold px-1.5 py-0.5 rounded-md bg-zinc-800 text-zinc-400 border border-zinc-700/50">
                    ...
                    <div className="flex items-center justify-between pt-2 border-t border-zinc-800/60 mt-auto">
                      <span className="text-[10px] text-zinc-600">
                        {entity.properties?.length ?? 0} propert{(entity.properties?.length ?? 0) === 1 ? "y" : "ies"}
                      </span>
                      {entity.version && (
        ...
        {tab === "relationships" && (
          <div className="flex flex-col gap-4">
            <div className="flex justify-end">
              <button id="new-relationship-btn" type="button" onClick={() => { setEditingRel(undefined); setRelFormOpen(true); }}
            ...
            {loading ? (
              <div className="flex items-center justify-center gap-2 py-12 text-zinc-500 text-xs"><Spinner /><span>Loading…</span></div>
            ) : relationships.length === 0 ? (
              <EmptyState
                icon={<svg width="36" height="36" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5"><line x1="5" y1="12" x2="19" y2="12"/><polyline points="13 6 19 12 13 18"/></svg>}
                title="No relationship types defined"
                sub="Define how entity types connect in the knowledge graph"
                action={
                  <button onClick={() => { setEditingRel(undefined); setRelFormOpen(true); }}
              ...
              <div className="border border-zinc-800/80 rounded-2xl overflow-hidden bg-zinc-950/40">
                ...
                <div className="divide-y divide-zinc-900">
                  {relationships.map((rel) => (
                    <div key={rel.id ?? rel.name} className="group grid grid-cols-12 px-5 py-3.5 items-center hover:bg-zinc-900/30 transition-all text-xs">
                      <div className="col-span-3">
                        <span className="font-bold font-mono text-purple-400">{rel.name}</span>
                        <span className="block text-[10px] text-zinc-500 mt-0.5">{rel.label}</span>
                      </div>
                      <div className="col-span-4 flex items-center gap-2">
                        <span className="font-semibold text-zinc-300 text-[11px]">{entityName(rel.sourceEntityTypeId)}</span>
                        ...
                        <span className="font-semibold text-zinc-300 text-[11px]">{entityName(rel.targetEntityTypeId)}</span>
                      </div>
                      ...
                      <div className="col-span-2">
                        <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-md border ${rel.directed ? "bg-blue-500/10 text-blue-400 border-blue-500/20" : "bg-zinc-800 text-zinc-500 border-zinc-700"}`}>
                          {rel.directed ? "Directed" : "Undirected"}
                        </span>
                      </div>
                      <div className="col-span-1 flex justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                        <button id={`edit-rel-${rel.id}`} type="button" onClick={() => { setEditingRel(rel); setRelFormOpen(true); }}
                        ...
                        {rel.id && (
                          <button id={`delete-rel-${rel.id}`} type="button" onClick={() => deleteRelationship(rel.id!)}
        ...
        {tab === "versions" && (
          <div className="flex flex-col gap-4">
            {versions.length === 0 ? (
              <EmptyState
                icon={<svg width="36" height="36" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5"><path d="M12 20h9"/><path d="M16.5 3.5a2.121 2.121 0 0 1 3 3L7 19l-4 1 1-4L16.5 3.5z"/></svg>}
              ...
              <div className="border border-zinc-800/80 rounded-2xl overflow-hidden bg-zinc-950/40">
                ...
                <div className="divide-y divide-zinc-900">
                  {versions.map((v) => (
                    <div key={v.id} className="grid grid-cols-12 px-5 py-3.5 items-center text-xs hover:bg-zinc-900/30 transition-all">
                      <div className="col-span-3 font-bold font-mono text-zinc-200">{v.version}</div>
                      <div className="col-span-4 text-zinc-500">{v.publishedAt}</div>
                      <div className="col-span-3 text-zinc-400">{v.publishedBy}</div>
                      <div className="col-span-2">
                        <span className={`text-[10px] font-bold px-2 py-0.5 rounded border ${
                          v.status === "PUBLISHED" ? "bg-emerald-500/10 text-emerald-400 border-emerald-500/20"
                          : v.status === "DEPRECATED" ? "bg-red-500/10 text-red-400 border-red-500/20"
